"""
FastAPI backend with Socket.io for Python-Duel real-time competitive coding platform.
Handles user connections, matchmaking, code submissions, and real-time updates.
"""
...
# Track connected users: {user_id: {socket_id, username, elo}}
connected_users: Dict[str, dict] = {}

# Track socket to user mapping: {socket_id: user_id}
...
async def join_queue(sid, data):
    ...
    Expected data: {user_id, challenge_id}
    """
    try:
        user_id = data.get('user_id')
...
async def submit_code(sid, data):
    ...
    Expected data: {user_id, room_id, code}
    """
    try:
        user_id = data.get('user_id')
...
async def sync_code(sid, data):
    """
    Broadcast code changes for real-time sync.
    Expected data: {user_id, room_id, code}
    """
    try:
        user_id = data.get('user_id')
        ...
        # Broadcast to opponent (to room, excluding sender)
